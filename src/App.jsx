...
const FlowerLogo = ({ size = 52 }) => (
  <img src={LOGO_SRC} alt="JM Flower Shop" width={size} height={size}
  ...
  return (
    <div>
      ...
      <div className="filter-bar">
        <input placeholder="Search arrangements..." value={searchQ} onChange={e => setSearchQ(e.target.value)} autoComplete="off" />
        ...
        <input placeholder="Max price ₱" value={filterMax} onChange={e => setFilterMax(e.target.value)} style={{ maxWidth:130 }} autoComplete="off" />
      </div>
      {filtered.length === 0
...
const CartView = ({ cart, updateQty, cartTotal, cartQty, grandTotal, deliveryFee,
  ...
  if (cart.length === 0) return (
    <div className="empty">
      <div className="empty-icon">🛒</div><p>Your cart is empty</p>
      ...
      <div className="card">
        ...
        <div className="form-group">
          ...
          <select value={deliveryTime} onChange={e => setDeliveryTime(e.target.value)}>
            {["","8:00 AM - 10:00 AM","10:00 AM - 12:00 PM","12:00 PM - 2:00 PM","2:00 PM - 4:00 PM","4:00 PM - 6:00 PM"].map(t => (
              <option key={t} value={t}>{t || "Select time slot"}</option>
            ))}
          </select>
        </div>
        {deliveryType === "delivery" && (
          <div className="form-group">
            ...
            <textarea placeholder="Enter full delivery address..." value={deliveryAddr} onChange={e => setDeliveryAddr(e.target.value)} autoComplete="off" />
          </div>
        )}
        <div className="form-group">
          <label>Gift Note (optional)</label>
          <textarea placeholder="Add a personal message..." value={giftNote} onChange={e => setGiftNote(e.target.value)} autoComplete="off" />
        </div>
        ...
        <div style={{ display:"flex",gap:10,marginTop:18 }}>
          ...
          <button className="btn btn-primary" style={{ flex:1 }} onClick={checkout} disabled={loading}>
      ...
      <div className="card">
        {cart.map(c => (
          <div className="cart-item" key={c.id}>
            ...
            <div style={{ minWidth:80,textAlign:"right",fontWeight:600,color:"var(--gold)",fontFamily:"Playfair Display,serif" }}>
              ₱{(parseFloat(c.price)*c.qty).toLocaleString()}
            </div>
          </div>
        ))}
        <div className="cart-total">
        ...
        <button className="btn btn-primary btn-full" style={{ marginTop:18 }} onClick={() => setCheckoutStep(2)}>
          Proceed to Checkout
        </button>
      </div>
    </div>
    ...
    <div>
      ...
      <div className="support-grid">
        <div className="support-opt">
          ...
          <a href="tel:09456475556" style={{ fontSize:12,color:"var(--text3)",marginTop:4,display:"block",textDecoration:"none" }}>09456475556</a>
        </div>
        <div className="support-opt">
          <div className="support-opt-icon">📧</div>
          <div className="support-opt-label">Email</div>
          <a href="mailto:Chanetics@gmail.com" style={{ fontSize:12,color:"var(--gold)",marginTop:4,display:"block",textDecoration:"none" }}>Chanetics@gmail.com</a>
        </div>
        <div className="support-opt">
          <div className="support-opt-icon">📘</div>
      ...
      <div className="card">
        <div style={{ textAlign:"center",marginBottom:22 }}>
          <FlowerLogo size={80} />
          <div style={{ fontFamily:"Playfair Display,serif",color:"var(--gold)",fontSize:16,fontWeight:700,marginTop:8 }}>JM Flower Shop</div>
        ...
        {tickets.length > 0 && (
          <div style={{ marginTop:26 }}>
            <div className="gold-line" />
            <h4 style={{ marginBottom:12,fontWeight:600,color:"var(--gold)",fontFamily:"Playfair Display,serif" }}>
            ...
            {tickets.map(t => (
              <div key={t.id} style={{ padding:14,background:"var(--black-soft)",border:"1px solid var(--border-gold)",borderRadius:10,marginBottom:12,fontSize:13 }}>
                ...
                <div style={{ fontWeight:600,color:"var(--text)",marginBottom:4 }}>{t.subject}</div>
                <div style={{ color:"var(--text2)",lineHeight:1.5 }}>{t.message}</div>
...
export default function App() {
  ...
  // Ping server immediately on load so it wakes up before user tries to log in
  useEffect(() => {
  ...
  const fetchProducts    = async () => { try { const r = await fetch(`${API}/products`); setProducts(await r.json()); } catch { showAlert("Cannot connect to server.", "error"); } };
  const fetchMyOrders    = async () => { try { const r = await fetch(`${API}/orders/user/${user.id}`); setOrders(await r.json()); } catch {} };
  const fetchAllOrders   = async () => { try { const r = await fetch(`${API}/orders`); setAllOrders(await r.json()); } catch {} };
  const fetchUsers       = async () => { try { const r = await fetch(`${API}/users`); setUsers(await r.json()); } catch {} };
  ...
  const login = async () => {
    ...
    try {
      ...
      if (!res.ok) { setAuthLoading(false); return showAlert(data.message || "Invalid login.", "error"); }
  ...
  const register = async () => {
    ...
    try {
      ...
      const data = await res.json();
      setServerStatus("ready");
      if (!res.ok) { setAuthLoading(false); return showAlert(data.message || "Registration failed.", "error"); }
      setUser(data); setModal(null); setView("shop");
  ...
  const logout = () => { setUser(null); setCart([]); setOrders([]); setTickets([]); setView("shop"); };
  ...
  const checkout = async () => {
    ...
    try {
      const payload = {
        ...
        items:cart, subtotal:cartTotal(), deliveryFee:deliveryFee(), total:grandTotal(),
        deliveryType, deliveryDate, deliveryTime:deliveryTime||"",
        deliveryAddr:deliveryAddr||"", giftNote:giftNote||""
      };
    ...
    } catch { showAlert("Failed to place order. Check your connection.", "error"); }
    setLoading(false);
  };
  ...
  const adminUpdateTicketStatus = async (ticketId, status) => {
    try {
      ...
      fetchAllTickets();
    } catch { showAlert("Failed to update ticket status.", "error"); }
  };
    ...
    <>
      ...
      <div id="app">
        ...
        <div className="main">
          ...
          {view === "shop" && (
            <ShopView
              products={products} user={user} setModal={setModal} addToCart={addToCart}
          ...
          {view === "cart" && (
            <CartView
              cart={cart} updateQty={updateQty} cartTotal={cartTotal} cartQty={cartQty}
          ...
          {view === "support" && (
            <SupportView
              user={user}
              supportSubj={supportSubj} setSupportSubj={setSupportSubj}
        ...
        {modal === "login" && (
          <div className="modal-overlay" onClick={e => e.target.classList.contains("modal-overlay") && !authLoading && setModal(null)}>
            <div className="modal">
              <div style={{ display:"flex",justifyContent:"center",marginBottom:16 }}><FlowerLogo size={90} /></div>
              <div className="modal-title">Welcome Back</div>

              {/* Server status banner */}
              {serverStatus === "waking" && (
                <div style={{ background:"#1a1200",border:"1px solid var(--gold-dark)",borderRadius:8,padding:"10px 14px",marginBottom:16,fontSize:13,color:"var(--gold)",display:"flex",alignItems:"center",gap:8 }}>
                  <span style={{ animation:"spin 1s linear infinite",display:"inline-block" }}>⏳</span>
                  Server is waking up, please wait a moment...
                </div>
              )}
              {serverStatus === "ready" && (
                <div style={{ background:"var(--success)",border:"1px solid #4a8a4a",borderRadius:8,padding:"10px 14px",marginBottom:16,fontSize:13,color:"#7ec87e",display:"flex",alignItems:"center",gap:8 }}>
                  ✅ Server is ready!
                </div>
              )}

              <div className="form-group"><label>Email</label>
                <input type="email" placeholder="your@email.com" value={loginEmail} onChange={e => setLoginEmail(e.target.value)} disabled={authLoading} />
              </div>
              <div className="form-group"><label>Password</label>
                <input type="password" placeholder="Password" value={loginPass} onChange={e => setLoginPass(e.target.value)} onKeyDown={e => e.key==="Enter" && !authLoading && login()} disabled={authLoading} />
              </div>
              <button className="btn btn-primary btn-full" onClick={login} disabled={authLoading}>
                {authLoading ? "⏳ Logging in..." : "Login"}
              </button>
              <div className="gold-line" />
              <p style={{ textAlign:"center",fontSize:13,color:"var(--text3)" }}>
                No account? <span style={{ color:"var(--gold)",cursor:"pointer",fontWeight:500 }} onClick={() => !authLoading && setModal("register")}>Register here</span>
              </p>
          ...
          <div className="modal-overlay" onClick={e => e.target.classList.contains("modal-overlay") && !authLoading && setModal(null)}>
            <div className="modal">
              ...
              <div className="form-group"><label>Full Name</label>
                <input placeholder="Your name" value={regName} onChange={e => setRegName(e.target.value)} disabled={authLoading} />
              </div>
              <div className="form-group"><label>Email</label>
                <input type="email" placeholder="your@email.com" value={regEmail} onChange={e => setRegEmail(e.target.value)} disabled={authLoading} />
              </div>
              <div className="form-group"><label>Password</label>
                <input type="password" placeholder="Password" value={regPass} onChange={e => setRegPass(e.target.value)} onKeyDown={e => e.key==="Enter" && !authLoading && register()} disabled={authLoading} />
              </div>
              <button className="btn btn-primary btn-full" onClick={register} disabled={authLoading}>
                {authLoading ? "⏳ Creating account..." : "Create Account"}
              </button>
              <div className="gold-line" />
              <p style={{ textAlign:"center",fontSize:13,color:"var(--text3)" }}>
                Have an account? <span style={{ color:"var(--gold)",cursor:"pointer",fontWeight:500 }} onClick={() => !authLoading && setModal("login")}>Sign in</span>
              </p>
            </div>
          </div>
        )}
